...
# 基于github上rouge包进行修改，原包见https://github.com/pltrdy/rouge
...
class FilesRouge:
  ...
  def get_scores(self, avg=False):
    ...
    if batch_lines is None:
      hyps = [line[:-1] for line in open(hyp_path, encoding='utf-8').readlines()]
      refs = [line[:-1] for line in open(ref_path, encoding='utf-8').readlines()]
...
class Rouge:
  ...
  def get_scores(self, hyps, refs, avg=False):
    ...
    if not avg:
      return self._my_get_scores(hyps, refs)
    return self._get_avg_scores(hyps, refs)

  def _my_get_scores(self, hyps, refs):
    """ 和参考摘要各句都进行对比，选择最大F值结果 """
    scores = []
    for hyp in hyps:
      sen_score = {}
      for m in self.metrics:
        fn = Rouge.AVAILABLE_METRICS[m]
        scs = [fn(hyp, ref) for ref in refs]
        sc = max(scs, key=lambda r:r[Rouge.AVAILABLE_STATS['r']])
